...
TargetTraitsA = {species={temperature="Normal",humidity="Damp",name="Clay",uid="gregtech.bee.speciesClay"}}
TargetTraitsB = {effect="forestry.allele.effect.none"}

function IsSubset(a, b)
    --check if table a is a subset of table b
    for k, v in pairs(a) do
        if type(v) == "table" then
            --compare tables
            if not b[k] or type(b[k]) ~= "table" then return false end
            for kk, vv in pairs(v) do
                if b[k][kk] ~= vv then return false end
            end
...
function IsBeeMissingBTrait(bee)
    --check if bee b is missing any traits from target in both active and inactive states
    for k, v in pairs(TargetTraitsB) do
        if ~IsSubset(v,bee.active[k]) and ~IsSubset(v,bee.inactive[k]) then return true end
    end
    return false
end

function EvaluateBeeIndividual(b)
    --sum of how many traits match the target
    local score = 0
    for k, v in pairs(TargetTrait) do
        if type(v) == "table" then
            --compare tables
            if b[k] and type(b[k]) == "table" then
                local match = true
                for kk, vv in pairs(v) do
                    if b[k][kk] ~= vv then
                        match = false
                ...
                if match then score = score + 1 end
            end
        else
            --compare simple values
            if b[k] == v then score = score + 1 end
        end
    end
    return score
end
...
--loop over every inventory slot and write its display name
for i=0,26 do
    if BeeChest[i] then
        local bee = BeeChest[i]
        if bee.individual and bee.individual.displayName then
            print("Slot " .. i .. ": " .. bee.individual.displayName)
        else
            print("Slot " .. i .. ": Empty or no display name")
        end
    else
        print("Slot " .. i .. ": Empty")
    end
